...
struct fraction add1(struct fraction f,struct fraction g)
{
	...
	int sum;
	sum=((f.numerator*g.denominator)+(g.numerator*f.denominator))/(f.denominator*g.denominator);
	return sum;
}
/*struct fraction add2(struct fraction *f,struct fraction *g)
{
	int sumn,sumd; 
	sumn=(f->numerator*g->denominator)+(g->numerator*f->denominator);
	sumd=(f->denominator*g->denominator);
